    ...
    def get_ai_analysis_summary(self) -> Dict:
        ...
        if not self.ai_analysis_history:
            return {
                ...
                'total_analyses': 0,
                'message': 'No AI analyses recorded'
            }

        total = len(self.ai_analysis_history)

        # 일치도 계산
        matches = sum(1 for a in self.ai_analysis_history
                     if a['v34_state'] == a['ai_state'])
        match_rate = matches / total if total > 0 else 0

        # 고신뢰도 분석 비율
        high_confidence = sum(1 for a in self.ai_analysis_history
                             if a['ai_confidence'] >= self.analyzer_v2.confidence_threshold)
        high_conf_rate = high_confidence / total if total > 0 else 0

        # 평균 신뢰도
        avg_confidence = np.mean([a['ai_confidence'] for a in self.ai_analysis_history])

        # AI 상태 분포
        ai_states = [a['ai_state'] for a in self.ai_analysis_history]
        state_counts = pd.Series(ai_states).value_counts().to_dict()

        return {
            'ai_enabled': self.ai_enabled,
            'ai_test_mode': self.ai_test_mode,
            'total_analyses': total,
            'v34_ai_match_rate': match_rate,
            'high_confidence_rate': high_conf_rate,
            'avg_confidence': avg_confidence,
            'confidence_threshold': self.analyzer_v2.confidence_threshold,
            'state_distribution': state_counts,
            'agents_enabled': self.analyzer_v2.agents_enabled
        }
