# 📊 데이터 확보 및 보충 자동화 도구

2024~2025년 전체 타임프레임 데이터를 완전하게 확보하기 위한 자동화 스크립트입니다.

---

## 🎯 목적

- **문제**: minute5는 2024-08-26부터 시작 (1~8월 누락)
- **목표**: 2024-01-01 ~ 2025-12-31 전체 데이터 확보
- **방법**: 업비트 API 수집 + 선형보간

---

## 📁 파일 구조

```
automation/
├── README.md                      # 이 파일
├── verify_all_timeframes.py       # 1. 데이터 검증 도구
├── collect_missing_data.py        # 2. 누락 데이터 수집
└── interpolate_gaps.py            # 3. 선형보간 처리
```

---

## 🚀 사용 방법

### Step 1: 데이터 검증

**현재 DB 상태를 확인하고 누락 구간을 식별합니다.**

```bash
cd /Users/bongbong/SynologyDrive/vendor/sandbox/251015_봉봇
python automation/verify_all_timeframes.py
```

**출력 예시:**

```
============================================================
📊 검증 요약
============================================================

완전한 타임프레임: 0/6
  ❌ minute5     :  56.93% (누락: 90,560개)
  ❌ minute15    :  89.66% (누락: 7,247개)
  ❌ minute30    :  89.66% (누락: 3,623개)
  ❌ minute60    :  89.66% (누락: 1,811개)
  ❌ minute240   :  89.66% (누락: 453개)
  ❌ day         :  89.73% (누락: 75개)

✅ 검증 리포트 저장: data_gap_report.json
```

**생성 파일:**

- `data_gap_report.json` - 상세 검증 결과

---

### Step 2: 누락 데이터 수집

**업비트 API를 통해 누락된 데이터를 수집합니다.**

```bash
python automation/collect_missing_data.py
```

**특징:**

- ✅ 자동으로 기존 데이터와 비교하여 누락분만 수집
- ✅ 중단해도 진행된 데이터는 저장됨
- ✅ 재실행 시 이어서 수집 (중복 체크)
- ✅ API Rate Limit 자동 준수 (초당 10회)

**예상 소요 시간:**

- minute5: ~1시간 (90,560개 캔들)
- minute15: ~15분 (7,247개 캔들)
- minute30: ~8분 (3,623개 캔들)
- minute60: ~4분 (1,811개 캔들)
- minute240: ~1분 (453개 캔들)
- day: ~10초 (75개 캔들)

**주의사항:**

- 실행 중 `Ctrl+C`로 언제든지 중단 가능
- 중단 시에도 진행된 데이터는 보존됨
- 재실행하면 이어서 수집됨

---

### Step 3: 선형보간 처리

**API에서도 없는 미세 누락 구간을 선형보간으로 채웁니다.**

```bash
python automation/interpolate_gaps.py
```

**처리 방법:**

```python
# 이전 캔들: 시가 100,000원
# 다음 캔들: 시가 102,000원
# 누락 캔들 2개

# 보간 결과:
# 누락 1: 100,000 + (102,000 - 100,000) * 1/3 = 100,667원
# 누락 2: 100,000 + (102,000 - 100,000) * 2/3 = 101,333원
```

**보간 데이터 표시:**

- DB 컬럼 `is_interpolated = 1`로 표시
- 원본 데이터와 구분 가능

---

### Step 4: 최종 검증

**모든 데이터가 완전한지 재확인합니다.**

```bash
python automation/verify_all_timeframes.py
```

**성공 기준:**

```
완전한 타임프레임: 6/6
  ✅ minute5     : 100.00% (누락: 0개)
  ✅ minute15    : 100.00% (누락: 0개)
  ✅ minute30    : 100.00% (누락: 0개)
  ✅ minute60    : 100.00% (누락: 0개)
  ✅ minute240   : 100.00% (누락: 0개)
  ✅ day         : 100.00% (누락: 0개)
```

---

## 🔄 전체 실행 예시

```bash
cd /Users/bongbong/SynologyDrive/vendor/sandbox/251015_봉봇

# 1. 현재 상태 확인
python automation/verify_all_timeframes.py

# 2. 데이터 백업 (선택사항)
cp upbit_bitcoin.db upbit_bitcoin_backup_$(date +%Y%m%d).db

# 3. 누락 데이터 수집 (시간 소요)
python automation/collect_missing_data.py

# 4. 선형보간 처리
python automation/interpolate_gaps.py

# 5. 최종 검증
python automation/verify_all_timeframes.py

# 6. DB 크기 확인
ls -lh upbit_bitcoin.db
```

---

## 📊 예상 결과

### 수집 전 (현재)

```
DB 크기: 489MB
minute5: 119,680개 (2024-08-26 ~ 2025-10-16)
```

### 수집 후 (예상)

```
DB 크기: ~520MB (+31MB)
minute5: 210,240개 (2024-01-01 ~ 2025-12-31) ✅
minute15: 70,080개 (2024-01-01 ~ 2025-12-31) ✅
minute30: 35,040개 (2024-01-01 ~ 2025-12-31) ✅
minute60: 17,520개 (2024-01-01 ~ 2025-12-31) ✅
minute240: 4,380개 (2024-01-01 ~ 2025-12-31) ✅
day: 730개 (2024-01-01 ~ 2025-12-31) ✅
```

---

## ⚠️ 주의사항

### 1. API Rate Limit

- 업비트: 초당 10회 제한
- 스크립트에 자동으로 구현됨 (`time.sleep(0.1)`)

### 2. 디스크 용량

```bash
# 실행 전 확인
df -h .
# 최소 1GB 여유 공간 필요
```

### 3. 중단 및 재실행

- `Ctrl+C`로 안전하게 중단 가능
- 진행된 데이터는 자동 저장
- 재실행 시 중복 체크로 이어서 수집

### 4. 데이터 백업

```bash
# 수집 전 백업 권장
cp upbit_bitcoin.db upbit_bitcoin_backup.db
```

---

## 🛠️ 문제 해결

### Python 모듈 없음

```bash
cd upbit_history_db
source venv/bin/activate
pip install -r requirements.txt
```

### Permission denied

```bash
chmod +x automation/*.py
```

### DB locked 에러

```bash
# 다른 프로세스가 DB를 사용 중
ps aux | grep python
kill <PID>
```

### 수집 중단 후 재개

```bash
# 단순히 다시 실행
python automation/collect_missing_data.py
# 자동으로 중복 체크하여 누락분만 수집
```

---

## 📈 진행 상황 모니터링

### 실시간 로그 확인

```bash
# 수집 중인 다른 터미널에서
watch -n 5 "sqlite3 upbit_bitcoin.db 'SELECT COUNT(*) FROM bitcoin_minute5'"
```

### DB 크기 모니터링

```bash
watch -n 30 "ls -lh upbit_bitcoin.db"
```

---

## 🔍 추가 도구

### 특정 타임프레임만 수집

```python
# collect_missing_data.py 수정
# collector.collect_all_timeframes() 대신:
collector.collect_all_data('minute5')
```

### 특정 기간만 수집

```python
# upbit_bitcoin_collector.py의 collect_all_data 메서드 수정
# 2019년 체크 부분을 원하는 날짜로 변경
```

---

## 📝 참고 사항

### 보간 데이터 식별

```sql
-- 보간된 데이터만 조회
SELECT * FROM bitcoin_minute5
WHERE is_interpolated = 1
ORDER BY timestamp DESC
LIMIT 10;

-- 보간 비율 확인
SELECT
    COUNT(*) as total,
    SUM(CASE WHEN is_interpolated = 0 THEN 1 ELSE 0 END) as original,
    SUM(CASE WHEN is_interpolated = 1 THEN 1 ELSE 0 END) as interpolated,
    ROUND(SUM(CASE WHEN is_interpolated = 1 THEN 1 ELSE 0 END) * 100.0 / COUNT(*), 2) as interpolation_rate
FROM bitcoin_minute5;
```

### 기존 수집기 직접 사용

```bash
cd upbit_history_db
source venv/bin/activate
python upbit_bitcoin_collector.py
```

---

## ✅ 완료 체크리스트

- [ ] Step 1: 검증 실행 → `data_gap_report.json` 생성
- [ ] Step 2: 데이터 수집 (약 1~2시간)
- [ ] Step 3: 선형보간 처리
- [ ] Step 4: 최종 검증 (모든 타임프레임 100%)
- [ ] DB 백업 생성
- [ ] 디스크 용량 확인

---

**작성일**: 2025-10-17
**버전**: 1.0
**작성자**: Claude (Automation Scripts)
