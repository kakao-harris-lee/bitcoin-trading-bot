...
class BasicTrendAgent:
    ...
    최적화 파라미터 (2025-11-13, Optuna 100 trials):
    ...
    def __init__(self):
        ...
        # Optuna 최적화 파라미터 (2025-11-13)
        self.sma_short_period = 22
        self.sma_long_period = 50
        self.strong_threshold = 0.028456707088086338
        self.confidence_strong = 0.9397771074506667
        self.confidence_weak = 0.7431264066149119
        self.confidence_sideways = 0.5616794696232922

    def analyze(self, df: pd.DataFrame) -> Dict[str, Any]:
        """기본 트렌드 분석 (최적화 버전)"""
        if len(df) < max(self.sma_long_period, 50):
            return {
                'market_state': 'SIDEWAYS_NEUTRAL',
                'confidence': self.confidence_sideways,
                'trend_strength': 0.0
            }

        # SMA 기반 트렌드 분석 (최적화된 기간)
        sma_short = df['close'].rolling(self.sma_short_period).mean().iloc[-1]
        sma_long = df['close'].rolling(self.sma_long_period).mean().iloc[-1]
        current_price = df['close'].iloc[-1]
        ...
        if current_price > sma_short > sma_long:
            if trend_strength > self.strong_threshold:
                state = 'BULL_STRONG'
