#!/usr/bin/env python3
"""
Binance V35 Parameter Optimizer

Optimizes V35 strategy parameters using Binance OHLCV data.
Uses Optuna for Bayesian hyperparameter optimization.

Usage:
    python scripts/optimize_v35_binance.py BTC --trials 500
    python scripts/optimize_v35_binance.py ETH --trials 200 --timeframe minute60
    python scripts/optimize_v35_binance.py BTC --validate  # Run validation only
"""
...
# Binance asset configurations
ASSET_CONFIG = {
    "BTC": {
        "db_path": "data/binance_bitcoin.db",
        "table_prefix": "binance",
        "config_path": "config/strategies/v35_btc_binance.json",
    },
    "ETH": {
        "db_path": "data/binance_ethereum.db",
        "table_prefix": "ethereum",
        "config_path": "config/strategies/v35_eth_binance.json",
    },
    "SOL": {
        "db_path": "data/binance_solana.db",
        "table_prefix": "solana",
        "config_path": "config/strategies/v35_sol_binance.json",
    },
}


class BinanceDataLoader:
    """Load Binance OHLCV data from SQLite database."""
...
class V35StrategyOptimizable:
    ...
    def check_exit(self, row: pd.Series) -> Optional[Dict]:
        ...
        if p.get('trailing_enabled', True) and pnl_pct > p.get('trailing_activation', 0.02):
            trail_pct = (close - self.high_water_mark) / self.high_water_mark
            if trail_pct < -p.get('trailing_distance', 0.015):
                return {
                    'action': 'sell',
                    'fraction': 1.0,
                    'reason': f'TRAILING_STOP: {pnl_pct*100:.2f}%'
                }
...
def run_backtest(df: pd.DataFrame, params: Dict[str, Any],
    ...
    for i in range(50, len(df)):
        ...
        strategy.market_state = strategy.classify_market(row)

        # Record equity
        position_value = position * close if position > 0 else 0
        ...
        if strategy.in_position:
            ...
            if exit_signal:
                sell_qty = position * exit_signal['fraction']
                ...
                if exit_signal['fraction'] >= 1.0 or position < 0.0001:
                    strategy.in_position = False
                    strategy.entry_price = 0
                    strategy.high_water_mark = 0
                    strategy.partial_exits = 0
                    position = 0
                else:
                    strategy.partial_exits += 1

        # Check entry
        if not strategy.in_position and capital > 0:
            entry_signal = strategy.check_entry(row)
            if entry_signal:
                buy_value = capital * entry_signal['fraction']
                buy_price = close * (1 + slippage)
                buy_qty = (buy_value * (1 - fee_rate)) / buy_price
                ...
                strategy.high_water_mark = buy_price
    ...
    # Final equity
    final_position_value = position * df.iloc[-1]['close'] if position > 0 else 0
    ...
    if len(equity_df) > 0:
        ...
        dd = (equity_df['total_equity'] - peak) / peak
        mdd = float(dd.min() * 100)

        # Sharpe ratio (annualized)
        returns = equity_df['total_equity'].pct_change().dropna()
        if len(returns) > 0 and returns.std() > 0:
            sharpe = float((returns.mean() / returns.std()) * np.sqrt(252 * 24))  # hourly data
        else:
            sharpe = 0
...
def create_objective(loader: BinanceDataLoader, timeframe: str,
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        params = {
            ...
            'mfi_bull_moderate': trial.suggest_int('mfi_bull_moderate', 42, 55),
            'mfi_sideways': trial.suggest_int('mfi_sideways', 38, 50),
            'adx_strong': trial.suggest_int('adx_strong', 18, 30),
            'adx_moderate': trial.suggest_int('adx_moderate', 12, 24),

            # Entry conditions
            'rsi_oversold': trial.suggest_int('rsi_oversold', 25, 45),
            'bb_mult': trial.suggest_float('bb_mult', 1.5, 2.5),
            'bb_entry_tolerance': trial.suggest_float('bb_entry_tolerance', 0.005, 0.05),
            ...
            # Position sizing
            'position_size_bull_strong': trial.suggest_float('position_size_bull_strong', 0.3, 0.8),
            'position_size_bull_moderate': trial.suggest_float('position_size_bull_moderate', 0.2, 0.6),
            'position_size_sideways': trial.suggest_float('position_size_sideways', 0.1, 0.4),

            # Exit conditions - Stop loss
            'stop_loss': trial.suggest_float('stop_loss', -0.05, -0.01),

            # Trailing stop
            'trailing_enabled': trial.suggest_categorical('trailing_enabled', [True, False]),
            'trailing_activation': trial.suggest_float('trailing_activation', 0.01, 0.03),
            'trailing_distance': trial.suggest_float('trailing_distance', 0.01, 0.025),

            # Take profit levels
            'tp_bull_strong_1': trial.suggest_float('tp_bull_strong_1', 0.02, 0.08),
            'tp_bull_strong_2': trial.suggest_float('tp_bull_strong_2', 0.06, 0.15),
            'tp_bull_strong_3': trial.suggest_float('tp_bull_strong_3', 0.12, 0.30),
            'tp_bull_moderate_1': trial.suggest_float('tp_bull_moderate_1', 0.015, 0.05),
            'tp_bull_moderate_2': trial.suggest_float('tp_bull_moderate_2', 0.04, 0.10),
            'tp_bull_moderate_3': trial.suggest_float('tp_bull_moderate_3', 0.08, 0.18),
            'tp_sideways_1': trial.suggest_float('tp_sideways_1', 0.01, 0.03),
            'tp_sideways_2': trial.suggest_float('tp_sideways_2', 0.025, 0.06),
            'tp_sideways_3': trial.suggest_float('tp_sideways_3', 0.05, 0.12),

            # Exit fractions
            'exit_fraction_1': trial.suggest_float('exit_fraction_1', 0.3, 0.5),
            'exit_fraction_2': trial.suggest_float('exit_fraction_2', 0.3, 0.5),
            'min_profit_for_macd_exit': trial.suggest_float('min_profit_for_macd_exit', 0.002, 0.015),
        }
        ...
        score = (
            total_return * 1.0 +           # Return weight
            mdd * 0.3 +                     # MDD penalty (negative value)
            sharpe * 10 +                   # Sharpe boost
            win_rate * 20                   # Win rate boost
        )
...
def run_optimization(symbol: str, timeframe: str = "minute60",
                     n_trials: int = 500, train_end: str = "2024-12-31",
                     val_start: str = "2025-01-01") -> Dict:
    """Run parameter optimization for an asset."""

    if not OPTUNA_AVAILABLE:
        raise RuntimeError("Optuna is required. Install with: pip install optuna")

    loader = BinanceDataLoader(symbol)
    ...
    print(f"\n{'='*60}")
    print(f"Optimizing V35 for {symbol} (Binance)")
    print(f"{'='*60}")
    ...
    study = optuna.create_study(
        ...
        study_name=f"v35_{symbol.lower()}_binance"
    )

    # Run optimization
    objective = create_objective(loader, timeframe, train_start, train_end)
    ...
    result = run_backtest(df_val, best_params)

    print(f"OOS Return: {result['total_return']:+.2f}%")
    print(f"OOS MDD: {result['mdd']:.2f}%")
    print(f"OOS Sharpe: {result['sharpe']:.2f}")
    print(f"OOS Trades: {result['total_trades']}")
    print(f"OOS Win Rate: {result['win_rate'] * 100:.1f}%")
...
def save_config(symbol: str, result: Dict):
    ...
    config_data = {
        "strategy_name": "v35_optimized_binance",
        "version": "v35",
        "asset": symbol,
        "exchange": "binance",
        "timeframe": result["timeframe"],
        "description": f"V35 Optuna optimized for {symbol} on Binance",
...
def main():
    parser = argparse.ArgumentParser(description="Optimize V35 parameters for Binance data")
    parser.add_argument('symbol', type=str, help='Asset symbol (BTC, ETH, SOL)')
    ...
    parser.add_argument('--train-end', type=str, default='2024-12-31', help='Training end date')
    parser.add_argument('--val-start', type=str, default='2025-01-01', help='Validation start date')
    ...
    if args.validate:
        ...
        # Run validation
        loader = BinanceDataLoader(symbol)
        _, end_date = loader.get_date_range(args.timeframe)

        df_val = loader.load(args.timeframe, args.val_start, end_date)
        result = run_backtest(df_val, params)

        print(f"\n{symbol} V35 Binance Validation Results")
        print(f"{'='*40}")
        print(f"Period: {args.val_start} to {end_date}")
