...
from strategy import add_all_indicators, v15_strategy
...
def run_backtest():
    ...
    print(f"지표 추가 완료: ADX, VWAP, OBV, Breakout, Bollinger, Stochastic")

    # 백테스팅
    capital = config['initial_capital']
    krw = capital
    btc = 0.0
    position = None  # {'entry_price', 'entry_date', 'highest_price', 'regime', 'algorithm', 'trailing_stop'}
    trades = []
    ...
    print(f"\n초기 자본: {capital:,}원")
    print(f"Trailing Stop: 동적 (전략별 상이)")
    ...
    for i in range(len(df)):
        ...
        # 포지션 없을 때 (매수 확인)
            ...
            signal = v15_strategy(df, i, config)
    ...
    if len(trades) > 0:
        ...
        for i, trade in enumerate(trades, 1):
            ...
            print(f"    시장 상황: {trade['regime']} | 알고리즘: {trade['algorithm']}")
            print(f"    Trailing Stop: {trade['trailing_stop']*100:.0f}% | 최고가: {trade['highest_price']:,.0f}원")
