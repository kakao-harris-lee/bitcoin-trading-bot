...
from strategy import v27_strategy
...
def run_backtest(df, config):
    ...
    for i in range(26, len(df)):
        position_info = {'entry_price': entry_price} if position > 0 else None
        signal = v27_strategy(df, i, config, position_info)
    ...
    for i in range(26, len(df)):
        # 간단한 equity 추적
        if temp_position > 0:
            current_value = temp_position * df.iloc[i]['close']
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"v27 백테스팅: {config['description']}")
    print(f"{'='*80}")
    ...
    if len(df_test) > 0:
        ...
        print(f"  Max Drawdown: {results_test['max_drawdown']:.2f}%")
    else:
        print("  데이터 없음")
        results_test = None
    ...
    print(f"Buy&Hold 대비: {avg_3year_return - buyhold_return:.2f}%p")
