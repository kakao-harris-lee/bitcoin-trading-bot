#!/usr/bin/env python3
"""
v40 Adaptive Voting Ensemble - Backtest Script

v39 + 손절 규칙 + Kelly Criterion
...
from strategy import V40AdaptiveVoting
...
def run_yearly_backtest(year: int, config: dict, db_path='../../upbit_bitcoin.db'):
    ...
    # 전략 설정
    strategy = V40AdaptiveVoting(config)
    ...
    # 손절 통계 추가
    stop_loss_stats = {
        'voting_stops': sum(1 for t in results['trades'] if 'VOTING_STOP' in str(t.reason)),
        'fixed_stops': sum(1 for t in results['trades'] if 'FIXED_STOP' in str(t.reason)),
        'time_stops': sum(1 for t in results['trades'] if 'TIME_STOP' in str(t.reason))
    }
    ...
    print(f"\n[손절 통계]")
    print(f"  다수결 손절: {stop_loss_stats['voting_stops']}회")
    print(f"  고정 손절 (-15%): {stop_loss_stats['fixed_stops']}회")
    print(f"  시간 손절 (90일): {stop_loss_stats['time_stops']}회")
...
def main():
    """메인 실행"""
    print("="*70)
    print("  v40 Adaptive Voting Ensemble - 연도별 백테스팅")
    print("  v39 + 손절 규칙 + Kelly Criterion")
    print("="*70)
    ...
    for year in years:
        ...
        if result:
            ...
            all_stop_stats[year] = result['stop_loss_stats']
    ...
    save_results = {
        ...
        'stop_loss_stats': {str(k): v for k, v in all_stop_stats.items()}
    }
    ...
    if len(all_results) > 0:
        ...
        avg_mdd = sum(m['max_drawdown'] for m in all_results.values()) / len(all_results)
        target_avg = config['target_performance']['annual_avg'] * 100
        target_sharpe = config['target_performance']['sharpe_ratio']
        target_mdd = config['target_performance']['max_drawdown'] * 100
        status_avg = "✅" if avg_return >= target_avg else "❌"
        status_sharpe = "✅" if avg_sharpe >= target_sharpe else "❌"
        status_mdd = "✅" if avg_mdd <= target_mdd else "❌"

        print(f"\n연평균 ({len(all_results)}년):")
        print(f"  목표: {target_avg:.0f}% | 실제: {avg_return:.2f}% {status_avg}")
        print(f"  평균 Sharpe: {avg_sharpe:.2f} (목표: {target_sharpe:.1f}) {status_sharpe}")
        print(f"  평균 MDD: {avg_mdd:.2f}% (목표: {target_mdd:.0f}%) {status_mdd}")

    # v39 vs v40 비교 (2022년 집중)
    if 2022 in all_results:
        print(f"\n" + "="*70)
        print("  v39 vs v40 비교 (2022년 손절 효과)")
        print("="*70)
        print(f"\nv39 (손절 없음):")
        print(f"  수익률: -28.16% | MDD: -31.17%")
        print(f"\nv40 (손절 있음):")
        print(f"  수익률: {all_results[2022]['total_return']:+.2f}% | MDD: {all_results[2022]['max_drawdown']:.2f}%")
        print(f"  손절 통계: 다수결 {all_stop_stats[2022]['voting_stops']}회, "
              f"고정 {all_stop_stats[2022]['fixed_stops']}회, "
              f"시간 {all_stop_stats[2022]['time_stops']}회")
        improvement = all_results[2022]['total_return'] - (-28.16)
        print(f"\n개선: {improvement:+.2f}%p")
