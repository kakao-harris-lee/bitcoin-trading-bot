#!/usr/bin/env python3
"""
v-a-05 Backtesting with Improved TP/SL
=======================================
...
class V37ExitManager:
    ...
    def _check_sideways_exit(self, row, prev_row, profit, highest_price, hold_days):
        """Sideways Exit (v05) - 3단계 TP 상향"""
        tp1 = self.config.get('sideways_tp_1', 0.03)  # 2% → 3%
        tp2 = self.config.get('sideways_tp_2', 0.05)  # 4% → 5%
        tp3 = self.config.get('sideways_tp_3', 0.08)  # 6% → 8%
...
def main():
    ...
    # 결과 저장
    output_file = Path(__file__).parent / 'results' / 'backtest_v05_results.json'
