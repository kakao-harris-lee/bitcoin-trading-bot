#!/usr/bin/env python3
"""
v-a-06: v37 Strategy Signal Generator
======================================
...
# v-a-06 로컬 모듈
va06_dir = Path(__file__).parent

# MarketClassifier
spec = importlib.util.spec_from_file_location(
    "market_classifier_v37",
    va06_dir / "core" / "market_classifier.py"
)
...
spec = importlib.util.spec_from_file_location(
    "dynamic_thresholds",
    va06_dir / "core" / "dynamic_thresholds.py"
)
...
spec = importlib.util.spec_from_file_location(
    "signal_extractors_v06",
    va06_dir / "signal_extractors_v06.py"
)
se_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(se_module)
TrendFollowingEntryCheckerV06 = se_module.TrendFollowingEntryCheckerV06
SwingTradingEntryCheckerV06 = se_module.SwingTradingEntryCheckerV06
SidewaysEntryCheckerV06 = se_module.SidewaysEntryCheckerV06
DefensiveEntryCheckerV06 = se_module.DefensiveEntryCheckerV06
apply_bear_market_filter_v06 = se_module.apply_bear_market_filter_v06


class V06SignalGenerator:
    ...
    def __init__(self, config: dict):
        ...
        self.trend_checker = TrendFollowingEntryCheckerV06(config)
        self.swing_checker = SwingTradingEntryCheckerV06(config)
        self.sideways_checker = SidewaysEntryCheckerV06(config)
        self.defensive_checker = DefensiveEntryCheckerV06(config)
    ...
    def generate_signals(self, df: pd.DataFrame, year: int) -> pd.DataFrame:
        ...
        for i in range(30, len(df)):
            ...
            # 3. v-a-06 핵심 개선: Bear Market Filter 적용
            # BEAR 시장에서 Sideways 진입 차단 → 2021-2022 손실 방지
            signal = apply_bear_market_filter_v06(signal, market_state)
...
def main():
    ...
    print("="*70)
    print("  v-a-06: v37 Strategy Signal Generator")
    print("="*70)
    ...
    for year in YEARS:
        ...
        # 시그널 생성
        generator = V06SignalGenerator(config)
