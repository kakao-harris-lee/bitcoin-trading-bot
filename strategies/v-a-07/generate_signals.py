#!/usr/bin/env python3
"""
v-a-07: Sideways 복원 + 선택적 BEAR 필터
========================================
v-a-06 실패 교훈 반영:
1. Sideways Volume 1.0x로 완화 → 시그널 복원
2. Bear Filter: BEAR 시장만 차단 (SIDEWAYS 시장 허용)
3. 목표: v-a-04 수준 (+56%) 유지 + 약세장 개선
"""
...
# v-a-07 로컬 모듈
va07_dir = Path(__file__).parent

# MarketClassifier
spec = importlib.util.spec_from_file_location(
    "market_classifier_v37",
    va07_dir / "core" / "market_classifier.py"
)
...
spec = importlib.util.spec_from_file_location(
    "dynamic_thresholds",
    va07_dir / "core" / "dynamic_thresholds.py"
)
...
# Stateless Signal Extractors v07
spec = importlib.util.spec_from_file_location(
    "signal_extractors_v07",
    va07_dir / "signal_extractors_v07.py"
)
se_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(se_module)
TrendFollowingEntryCheckerV07 = se_module.TrendFollowingEntryCheckerV07
SwingTradingEntryCheckerV07 = se_module.SwingTradingEntryCheckerV07
SidewaysEntryCheckerV07 = se_module.SidewaysEntryCheckerV07
DefensiveEntryCheckerV07 = se_module.DefensiveEntryCheckerV07
apply_bear_market_filter_v07 = se_module.apply_bear_market_filter_v07


class V07SignalGenerator:
    """v07 전략 시그널 생성기 (Sideways 복원 + 선택적 BEAR 필터)"""
    ...
    def __init__(self, config: dict):
        ...
        # 4종 무상태 Entry Checker (v07)
        self.trend_checker = TrendFollowingEntryCheckerV07(config)
        self.swing_checker = SwingTradingEntryCheckerV07(config)
        self.sideways_checker = SidewaysEntryCheckerV07(config)
        self.defensive_checker = DefensiveEntryCheckerV07(config)
    ...
    def generate_signals(self, df: pd.DataFrame, year: int) -> pd.DataFrame:
        ...
        for i in range(30, len(df)):
            ...
            # 3. v-a-07 핵심 개선: 선택적 Bear Market Filter 적용
            # BEAR_MODERATE/STRONG에서만 Sideways 차단 (SIDEWAYS 시장은 허용)
            signal = apply_bear_market_filter_v07(signal, market_state)
...
def main():
    ...
    print("="*70)
    print("  v-a-07: Sideways 복원 + 선택적 BEAR 필터")
    print("="*70)
    ...
    for year in YEARS:
        ...
        # 시그널 생성
        generator = V07SignalGenerator(config)
        ...
        signals_json = {
            'strategy': 'v-a-07',
            'version': '1.0',
            'description': 'Sideways Restoration + Selective BEAR Filter (v37 based)',
            'timeframe': TIMEFRAME,
            'year': year,
    ...
    print(f"\n다음 단계:")
    print(f"  1. python strategies/v-a-06/backtest.py (백테스팅)")
