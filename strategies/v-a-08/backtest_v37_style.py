...
class V37ExitManager:
    ...
    def _check_sideways_exit(self, row, prev_row, profit, highest_price, hold_days):
        """
        Sideways Exit v-a-08: ATR 기반 동적 TP/SL

        핵심 개선:
        - SL: 고정 -2% → ATR 1.5배
        - TP: 고정 2%/4%/6% → ATR 2/3/5배
        - Trailing: 고정 -1% → ATR 1배
        """
        # v-a-08: ATR 기반 동적 Exit
        atr = row.get('atr', 0)
        entry_price = highest_price / (1 + max(profit, 0))  # 역산

        # 동적 SL/TP (ATR 비율로 계산)
        atr_ratio = atr / entry_price

        stop_loss = -1.5 * atr_ratio  # ATR 1.5배
        tp1 = 2.0 * atr_ratio  # ATR 2배
        tp2 = 3.0 * atr_ratio  # ATR 3배
        tp3 = 5.0 * atr_ratio  # ATR 5배
        trailing_threshold = 1.0 * atr_ratio  # ATR 1배
        ...
        # Trailing Stop (v-a-08: ATR 기반)
        current_price = row['close']
        drawdown_from_peak = (current_price - highest_price) / highest_price

        if drawdown_from_peak <= -trailing_threshold:
            return {'should_exit': True, 'reason': 'SIDEWAYS_TRAILING_STOP', 'fraction': 1.0}
