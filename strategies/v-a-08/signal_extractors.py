...
class SidewaysEntryChecker:
    """Sideways Entry v-a-08: 변동성 필터 추가"""
    ...
    def check_entry(self, row: pd.Series, prev_row: pd.Series, df_recent: pd.DataFrame) -> Optional[Dict]:
        ...
        # ========== v-a-08 변동성 필터 ==========
        # 1. 20일 변동성 체크
        recent_volatility = df_recent[-20:]['close'].std() / row['close']
        vol_threshold = self.config.get('volatility_threshold', 0.08)  # 8%

        if recent_volatility > vol_threshold:
            return None  # 변동성 폭발 구간 진입 거부

        # 2. ATR 급등 체크
        atr = row.get('atr', 0)
        atr_avg = df_recent[-14:]['atr'].mean()

        if atr > atr_avg * 1.3:  # ATR 30% 이상 증가
            return None  # 변동성 급증 진입 거부
        # =====================================
