...
class SidewaysEntryChecker:
    """Sideways Entry v-a-09: OBV 기반 하락 추세 필터"""
    ...
    def check_entry(self, row: pd.Series, prev_row: pd.Series, df_recent: pd.DataFrame) -> Optional[Dict]:
        """
        v-a-09 핵심 개선: OBV 하락 추세 감지
        - v-a-08 실패 원인: 변동성만으로는 부족
        - 해결: OBV (매집/매도) 추세로 하락 방향성 감지
        ...
        Returns:
            None or {'action': 'buy', 'fraction': 0.4, 'reason': str}
        """
        if prev_row is None or len(df_recent) < 20:
            return None

        # ========== v-a-09: OBV 하락 추세 필터 ==========
        # OBV 계산 (On-Balance Volume)
        obv_series = []
        obv = 0
        for idx in range(len(df_recent)):
            curr_row = df_recent.iloc[idx]
            if idx > 0:
                prev = df_recent.iloc[idx-1]
                if curr_row['close'] > prev['close']:
                    obv += curr_row['volume']
                elif curr_row['close'] < prev['close']:
                    obv -= curr_row['volume']
            obv_series.append(obv)

        # OBV 14일 이동평균 기울기
        if len(obv_series) >= 14:
            obv_ma = pd.Series(obv_series).rolling(14).mean()
            obv_slope = (obv_ma.iloc[-1] - obv_ma.iloc[-7]) / obv_ma.iloc[-7]  # 7일 기울기

            # OBV 하락 추세 → 진입 거부
            if obv_slope < -0.05:  # -5% 이하
                return None  # 매도 압력 강함
        # ==============================================
