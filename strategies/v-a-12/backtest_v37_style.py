...
class V37ExitManager:
    ...
    def check_exit(
        ...
        highest_price: float,
        hold_days: int,
        market_state: str = 'SIDEWAYS'  # v-a-12: market_state 추가
    ) -> dict:
        """
        전략별 Exit 조건 체크 (v-a-12: Adaptive TP)
        ...
        if strategy == 'trend_following':
            ...
            return self._check_sideways_exit(current_row, prev_row, profit, highest_price, hold_days, market_state)  # market_state 전달
    ...
    def _check_sideways_exit(self, row, prev_row, profit, highest_price, hold_days, market_state='SIDEWAYS'):
        """
        Sideways Exit v-a-12: Adaptive Profit Taking

        핵심 개선 (v-a-11 → v-a-12):
        1. 시장 상태별 동적 TP 조정
           - BULL: 6%/12%/20% (v-a-11 공격적)
           - SIDEWAYS: 3%/6%/10% (중간)
           - BEAR: 2%/4%/6% (v-a-04 보수적)
        2. ATR 기반 동적 Trailing Stop

        목표: 2024년 +65% → +82% 회복
        """
        # v-a-12: Adaptive TP (시장 상태 기반)
        if market_state in ['BULL_STRONG', 'BULL_MODERATE']:
            # 상승장: 공격적 TP (v-a-11)
            tp1, tp2, tp3 = 0.06, 0.12, 0.20
        elif market_state == 'SIDEWAYS':
            # 횡보장: 중간 TP
            tp1, tp2, tp3 = 0.03, 0.06, 0.10
        else:  # BEAR
            # 약세장: 보수적 TP (v-a-04 원본)
            tp1, tp2, tp3 = 0.02, 0.04, 0.06

        stop_loss = self.config.get('sideways_stop_loss', -0.02)
        max_hold = self.config.get('sideways_max_hold_days', 30)
...
def backtest_year(year: int, config: dict, db_path: Path) -> dict:
    ...
    for _, signal in signals_df.iterrows():
        ...
        market_state = signal['market_state']  # v-a-12: market_state 추가
        ...
        for idx, row in future_df.iterrows():
            ...
            exit_check = exit_manager.check_exit(
                ...
                hold_days=hold_days,
                market_state=market_state  # v-a-12: market_state 전달
            )
