...
class V37ExitManager:
    ...
    def _check_bull_trend_exit(self, row, prev_row, profit, highest_price, hold_days):
        ...
        Max Hold: 40일
        """
        tp1 = self.config.get('bull_trend_take_profit_1', 0.08)
        tp2 = self.config.get('bull_trend_take_profit_2', 0.15)
        tp3 = self.config.get('bull_trend_take_profit_3', 0.25)
        stop_loss = self.config.get('bull_trend_stop_loss', -0.05)
        trailing_trigger = self.config.get('bull_trend_trailing_trigger', 0.10)
        trailing_stop = self.config.get('bull_trend_trailing_stop', -0.03)
        max_hold = self.config.get('bull_trend_max_hold_days', 40)
...
def main():
    ...
    print("="*70)
    print("  v-a-04 Backtesting (v37 Exit Logic)")
    print("="*70)
