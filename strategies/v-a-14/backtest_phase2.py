...
class V37ExitManager:
    ...
    def check_exit(
        ...
        if strategy == 'bull_trend':  # v-a-14 NEW!
            return self._check_bull_trend_exit(current_row, prev_row, profit, highest_price, hold_days)
    ...
    def _check_bull_trend_exit(self, row, prev_row, profit, highest_price, hold_days):
        """
        v-a-14: BULL Trend Exit (공격적)

        TP: 8%/15%/25%
        SL: -5%
        Trailing: 고점 -3% (10% 이상 수익 시)
        Max Hold: 40일
        """
        bull_config = self.config.get("bull_trend", {})
        tp1 = bull_config.get('take_profit_1', 0.08)
        tp2 = bull_config.get('take_profit_2', 0.15)
        tp3 = bull_config.get('take_profit_3', 0.25)
        stop_loss = bull_config.get('stop_loss', -0.05)
        trailing_trigger = bull_config.get('trailing_trigger', 0.10)
        trailing_stop = bull_config.get('trailing_stop', -0.03)
        max_hold = bull_config.get('max_hold_days', 40)

        # Progressive TP
        if profit >= tp3:
            return {'should_exit': True, 'reason': 'BULL_TREND_TP3', 'fraction': 1.0}
        if profit >= tp2:
            return {'should_exit': True, 'reason': 'BULL_TREND_TP2', 'fraction': 1.0}
        if profit >= tp1:
            return {'should_exit': True, 'reason': 'BULL_TREND_TP1', 'fraction': 1.0}

        # Stop Loss
        if profit <= stop_loss:
            return {'should_exit': True, 'reason': 'BULL_TREND_STOP_LOSS', 'fraction': 1.0}
        ...
        if profit >= trailing_trigger:
            ...
            if drawdown_from_peak <= trailing_stop:
                return {'should_exit': True, 'reason': 'BULL_TREND_TRAILING', 'fraction': 1.0}

        # Timeout
        if hold_days >= max_hold:
            return {'should_exit': True, 'reason': 'BULL_TREND_TIMEOUT', 'fraction': 1.0}

        return {'should_exit': False, 'reason': 'BULL_TREND_HOLD', 'fraction': 0}
...
def backtest_year(year: int, config: dict, db_path: Path) -> dict:
    ...
    print(f"    BULL Trend: {(signals_df['strategy']=='bull_trend').sum()}개")  # NEW!
...
def main():
    ...
    print("="*70)
    print("  v-a-14 Optimized Backtesting (Phase 2: Entry 강화 + TP 밸런스)")
    print("="*70)

    # Config 로드
    config_path = Path(__file__).parent / 'config_phase2.json'
    ...
    # 결과 저장
    output_file = Path(__file__).parent / 'results' / 'backtest_v37_style.json'
    output_file.parent.mkdir(exist_ok=True)

    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump({
            'strategy': 'v-a-04',
            'description': 'v37 Entry + v37 Exit',
