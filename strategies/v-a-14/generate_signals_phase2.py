#!/usr/bin/env python3
"""
v-a-14: Enhanced Signal Generator with BULL Trend Strategy
===========================================================
v-a-11 (SIDEWAYS + v37 strategies) + NEW BULL Trend Strategy

Strategies:
1. Trend Following (v37)
2. Swing Trading (v37)
3. Sideways (v-a-11 improved)
4. Defensive (v37)
5. BULL Trend (NEW!)
"""
...
# v-a-14 NEW: BULL Trend Strategy
spec = importlib.util.spec_from_file_location(
    "bull_trend",
    va04_dir / "strategies" / "bull_trend.py"
)
bt_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(bt_module)
BullTrendChecker = bt_module.BullTrendChecker


class V37SignalGenerator:
    """v-a-14: v37 + BULL Trend 전략 시그널 생성기"""

    def __init__(self, config: dict):
        """
        Args:
            config: v-a-14 config_phase2.json
        """
        self.config = config
        ...
        # 5종 무상태 Entry Checker (v-a-14: BULL Trend 추가!)
        ...
        self.bull_trend_checker = BullTrendChecker(config)  # NEW!
    ...
    def generate_signals(self, df: pd.DataFrame, year: int) -> pd.DataFrame:
        ...
        for i in range(30, len(df)):
            ...
            # v-a-14: BULL Trend (NEW!) - 최우선
            bull_trend_signal = self.bull_trend_checker.check_entry(row, prev_row, market_state)
            if bull_trend_signal:
                signal = bull_trend_signal
                signal['market_state'] = market_state

            # Trend Following (MACD GC + ADX >= 16)
            if signal is None:
