...
from strategy import AdaptiveTrendRiderStrategy, v04_strategy_wrapper
...
def run_backtest(
    ...
    Args:
        timeframe: 거래 타임프레임 (minute5, minute240 등)
        start_date: 시작일
        end_date: 종료일
        config_path: config.json 경로
    """
    print("="*80)
    print("v04 Adaptive Trend Rider Strategy Backtest")
    print("="*80)
    ...
    print(f"  - Day candles: {len(df_day):,}")
    print(f"  - Trade candles ({timeframe}): {len(df_trade):,}")
    print(f"  - Day index sync: {'✓' if 'day_idx' in df_trade.columns else '✗'}")
    ...
    strategy_params = {
        ...
        'backtester': backtester  # Backtester 인스턴스 전달 (자본 조회용)
    }
    ...
    # Buy&Hold 기준선
    start_price = df_trade.iloc[0]['close']
    end_price = df_trade.iloc[-1]['close']
    ...
    print(f"\n=== 백테스팅 기간 ===")
    print(f"시작: {df_trade.iloc[0]['timestamp']} (시작가: {start_price:,.0f}원)")
    print(f"종료: {df_trade.iloc[-1]['timestamp']} (종료가: {end_price:,.0f}원)")
    print(f"기간: {(pd.to_datetime(end_date) - pd.to_datetime(start_date)).days}일 | 캔들: {len(df_trade):,}개")
    ...
    print(f"\n=== 거래 통계 ===")
    print(f"총 거래: {metrics['total_trades']}회 | 승률: {metrics['win_rate']:.1f}%")
    ...
    result_filename = f'results_{timeframe}.json'
