...
class SimpleBacktester:
    ...
    def execute_sell(self, timestamp, price: float, fraction: float, reason: str = "") -> bool:
        """매도"""
        if self.position <= 0:
            return False
    ...
    def run(self, df: pd.DataFrame, strategy_func: Callable, params: dict) -> Dict:
        """
        백테스팅 실행

        Args:
            df: 데이터프레임 (지표 포함)
            strategy_func: 전략 함수 (df, i, params) -> {'action': 'buy'|'sell'|'hold', 'fraction': float}
            params: 전략 파라미터
        ...
        for i in range(len(df)):
            ...
            # 전략 결정
            decision = strategy_func(df, i, params)

            # 실행
            if decision['action'] == 'buy':
                fraction = decision.get('fraction', 1.0)
                self.execute_buy(timestamp, price, fraction)

            elif decision['action'] == 'sell':
                fraction = decision.get('fraction', 1.0)
                reason = decision.get('reason', '')
                self.execute_sell(timestamp, price, fraction, reason)
