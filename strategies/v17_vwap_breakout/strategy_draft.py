...
def add_indicators(df, config):
    ...
    from strategies._library.momentum.breakout_only import StochasticOscillator
    ...
    # Stochastic
    stoch = StochasticOscillator(**config['algorithms']['breakout_only']['params'])
...
def check_breakout_only_signal(df, i, params):
    """Stochastic 매수 신호 확인"""
...
def v17_strategy(df, i, config):
    ...
    stoch_signal = check_breakout_only_signal(df, i, config['algorithms']['breakout_only']['params'])
    ...
    if stoch_signal:
        score += config['algorithms']['breakout_only']['weight']
...
if __name__ == '__main__':
    ...
    # 신호 테스트
    print(f"Testing v17 Voting Ensemble...")
    signals_found = 0

    for i in range(len(df)):
        result = v17_strategy(df, i, config)
