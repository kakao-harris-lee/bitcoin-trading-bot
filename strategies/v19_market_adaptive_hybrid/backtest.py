#!/usr/bin/env python3
"""
v19 Market-Adaptive Hybrid 백테스팅
"""
...
from strategy import add_indicators, v19_strategy
...
def run_backtest(start_date='2022-01-01', end_date='2022-12-31'):
    ...
    print("="*80)
    print(f"v19 {config['strategy_name'].upper()} 백테스팅")
    print("="*80)
    ...
    print(f"Trailing Stop: {config['bull_strategy']['trailing_stop_pct']*100}%")
    print(f"Stop Loss (Bull): {config['bull_strategy']['stop_loss_pct']*100}%")
    ...
    for i in range(len(df)):
        ...
        # 전략 실행
        signal = v19_strategy(df, i, position, config)
        ...
        # 매도
            ...
            trade = {
                ...
                'market_type': position.get('market_type', 'UNKNOWN')
            }
            trades.append(trade)
