#!/usr/bin/env python3
"""
v38 Ensemble Strategy - Backtest Script
...
from strategies.v38_ensemble.strategy import V38EnsembleStrategy
...
def run_yearly_backtest(year: int, config: dict, db_path='../../upbit_bitcoin.db'):
    ...
    # 전략 설정
    strategy = V38EnsembleStrategy(config)
    ...
    print(f"\n[전략 활용 통계]")
    print(f"  전략 사용:")
    for strategy_name, count in strategy_stats['strategy_usage'].items():
        print(f"    {strategy_name}: {count}회")
    print(f"  시장 상태 분포:")
    for state, pct in strategy_stats['market_state_distribution'].items():
...
def main():
    """메인 실행"""
    print("="*70)
    print("  v38 Ensemble Strategy - 연도별 백테스팅")
    print("="*70)
