#!/usr/bin/env python3
"""
v41 S/A-Tier 단타 전략 백테스팅 (복리 오버플로우 수정)
- 안전한 복리 계산 (오버플로우 방지)
- 로그 스케일 수익률 계산
"""
...
# 오버플로우 방지 설정
MAX_CAPITAL = 1e15  # 최대 자본 (1,000조)
MAX_LOG_RETURN = 10.0  # 최대 로그 수익률 (e^10 = 22,026배)


class ScalpingBacktestFixed:
    def __init__(self, timeframe, year, tier='S'):
        self.timeframe = timeframe
        self.year = year
        self.tier = tier
        self.capital = INITIAL_CAPITAL
        self.log_capital = np.log(INITIAL_CAPITAL)  # 로그 스케일
        self.position = None
        self.trades = []
    ...
    def safe_update_capital(self, trade_pnl):
        """안전한 복리 계산 (오버플로우 방지)"""
        # 로그 스케일로 계산
        log_return = np.log1p(trade_pnl)  # log(1 + pnl)

        # 오버플로우 체크
        if log_return > MAX_LOG_RETURN:
            log_return = MAX_LOG_RETURN
            self.overflow_warning = True

        self.log_capital += log_return

        # 최대 자본 제한
        if self.log_capital > np.log(MAX_CAPITAL):
            self.log_capital = np.log(MAX_CAPITAL)
            self.overflow_warning = True

        # 실제 자본 계산
        self.capital = np.exp(self.log_capital)
    ...
    def run(self):
        ...
        if len(signals) == 0:
            return self.get_results([], INITIAL_CAPITAL, 0)
            ...
            if self.position:
                ...
                for _, row in future_prices.iterrows():
                    ...
                    if should_exit:
                        ...
                        # 안전한 복리 계산
                        self.safe_update_capital(trade_pnl)
...
def run_all_backtests():
    ...
    for timeframe in timeframes:
        for year in years:
            for tier in tiers:
                try:
                    bt = ScalpingBacktestFixed(timeframe, year, tier)
                    result = bt.run()
                    all_results.append(result)

                    # 요약 출력
                    overflow_mark = " ⚠️ OVERFLOW" if result['overflow_warning'] else ""
                    print(f"{timeframe} {year} {tier}-Tier: "
                          f"Return={result['total_return']:.2%}, "
                          f"Trades={result['total_trades']}, "
                          f"Win Rate={result['win_rate']:.1%}, "
                          f"Sharpe={result['sharpe_ratio']:.2f}{overflow_mark}")

                    # 개별 파일 저장
                    output_file = OUTPUT_DIR / f"backtest_{timeframe}_{year}_{tier}tier_fixed.json"
    ...
    # 전체 결과 저장
    summary_file = OUTPUT_DIR / "backtest_summary_fixed.json"
...
def create_summary_table(results):
    ...
    # CSV 저장
    df.to_csv(OUTPUT_DIR / 'backtest_summary_fixed.csv', index=False)
    tf_summary.to_csv(OUTPUT_DIR / 'timeframe_summary_fixed.csv', index=False)


if __name__ == '__main__':
    print("v41 S/A-Tier 단타 백테스팅 (복리 오버플로우 수정)")
    print(f"설정: TP={TAKE_PROFIT:.1%}, SL={STOP_LOSS:.1%}, Max Hold={MAX_HOLD_HOURS}h")
    print(f"안전장치: 최대 자본={MAX_CAPITAL:,.0f}원, 최대 로그 수익률={MAX_LOG_RETURN}")
    print("="*80)
    ...
    print("\n\n완료! 결과 파일:")
    print(f"  - {OUTPUT_DIR}/backtest_summary_fixed.csv")
    print(f"  - {OUTPUT_DIR}/timeframe_summary_fixed.csv")
    print(f"  - {OUTPUT_DIR}/backtest_summary_fixed.json")
