...
def test_update_cached_position_adds_and_removes():
    ...
    manager.update_cached_position("BTC", "mlp_direction_btc", position)
    assert "mlp_direction_btc" in manager.get_positions_for_symbol("BTC")
    assert len(manager.get_portfolio_positions()) == 1

    manager.update_cached_position("BTC", "mlp_direction_btc", None)
    assert "mlp_direction_btc" not in manager.get_positions_for_symbol("BTC")
