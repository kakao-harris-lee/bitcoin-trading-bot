...
def test_trading_context_creation():
    ...
    assert "llm_direction_btc" in ctx.positions
...
def test_trading_context_has_position():
    ...
    positions = {"llm_direction_btc": Position(symbol="BTC", entry_price=99000.0, quantity=0.01, strategy="llm_direction_btc", market="spot", timestamp=900)}

    ctx = TradingContext(symbol="BTC", timestamp=1000, market=market, regime=regime, positions=positions)

    assert ctx.has_position("llm_direction_btc") is True
    assert ctx.has_position("llm_direction_eth") is False
...
def test_trading_context_get_position():
    ...
    market = MarketData(symbol="BTC", close=100000.0, mfi=50.0, adx=20.0, rsi=50.0, timestamp=1000)
    regime = build_market_context(mfi=50.0, adx=20.0, atr=1000.0, close=100000.0)
    pos = Position(symbol="BTC", entry_price=99000.0, quantity=0.01, strategy="llm_direction_btc", market="spot", timestamp=900)
    positions = {"llm_direction_btc": pos}

    ctx = TradingContext(symbol="BTC", timestamp=1000, market=market, regime=regime, positions=positions)

    assert ctx.get_position("llm_direction_btc") == pos
...
def test_trading_context_other_strategies_positioned():
    ...
    regime = build_market_context(mfi=50.0, adx=20.0, atr=1000.0, close=100000.0)
    positions = {
        "llm_direction_btc": Position(symbol="BTC", entry_price=99000.0, quantity=0.01, strategy="llm_direction_btc", market="spot", timestamp=900),
        "sideways": Position(symbol="BTC", entry_price=98000.0, quantity=0.02, strategy="sideways", market="spot", timestamp=800),
    }

    ctx = TradingContext(symbol="BTC", timestamp=1000, market=market, regime=regime, positions=positions)

    others = ctx.other_strategies_positioned("llm_direction_btc")
    assert "sideways" in others
    assert "llm_direction_btc" not in others
